      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-stone-100 p-4">
          <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
            ...
            <h2 className="text-2xl font-bold text-stone-900 mb-2">Unexpected Error</h2>
...
function MainApp() {
  const [activeTab, setActiveTab] = useState<'planner' | 'shopping' | 'favorites' | 'profiles'>(
    'planner'
  );
  ...
  const [manualItems, setManualItems] = useState<
    { id: string; name: string; amount: string; unit: string; category: string }[]
  >([]);
  ...
  const emptyRecipeToAppRecipe = (dbRecipe: any): Recipe => {
    ...
    return {
      ...
      prepTime: dbRecipe.prep_time ?? 10,
      cookTime: dbRecipe.cook_time ?? 20,
      costPerServing: Number(dbRecipe.estimated_cost) || 2.5,
      ingredients: Array.isArray(dbRecipe.ingredients)
        ? dbRecipe.ingredients
        : [{ name: 'Ingredients not stored yet', amount: '1', unit: 'recipe', category: 'Cupboard' }],
  ...
  const buildMealPlanFromRecipes = (dbRecipes: any[]): MealPlanItem[] => {
    const slots: MealPlanItem[] = [];
    let index = 0;

    for (let day = 0; day < DAYS.length; day++) {
      for (const mealType of MEAL_TYPES) {
        const dbRecipe = dbRecipes[index];
        if (dbRecipe) {
          slots.push({
            day,
            mealType,
            recipe: emptyRecipeToAppRecipe(dbRecipe),
          });
        }
        index++;
      }
  ...
  const fetchRecipes = async () => {
    const { data, error } = await supabase
      .from('recipes')
      .select(
        'id, title, description, instructions, servings, image_url, created_at, prep_time, cook_time, estimated_cost'
      )
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        mealPlanRes.status === 401 ||
        favoritesRes.status === 401 ||
        shoppingRes.status === 401
      ) {
  ...
  useEffect(() => {
    if (recipes.length > 0) {
      setMealPlan(buildMealPlanFromRecipes(recipes));
    }
  }, [recipes]);
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'register') {
        ...
        if (data.user) {
          setToast({
            message: 'Welcome to Basket Wise! Please check your email for verification.',
            type: 'success',
          });
        }
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      ...
      for (let day = 0; day < DAYS.length; day++) {
        for (const mealType of MEAL_TYPES) {
          const prompt = `Create one ${mealType} recipe for ${DAYS[day]} for a family of ${adults} adults and ${children} children, with a total weekly budget of £${budget}. Preferences: ${filters.join(', ')}. Include a generated food photo.`;

          const result = await generateSingleRecipe(prompt);
          const savedRecipe = result?.recipe?.[0];

          if (savedRecipe) {
            generatedResults.push(savedRecipe);
          }
        }
      }
      ...
      setToast({
        message: 'Weekly plan generated and saved to Supabase!',
        type: 'success',
      });
    } catch (error: any) {
  ...
  const handleRegenerateMeal = async () => {
    ...
    try {
      const prompt = `Create one ${selectedMealInfo.type} recipe for ${DAYS[selectedMealInfo.day]} for a family of ${adults} adults and ${children} children, within a weekly budget of £${budget}. Preferences: ${filters.join(', ')}. Include a generated food photo.`;

      const result = await generateSingleRecipe(prompt);
      const savedRecipe = result?.recipe?.[0];

      if (!savedRecipe) {
        throw new Error('No saved recipe returned');
      }

      const updatedRecipe = emptyRecipeToAppRecipe(savedRecipe);

      const newPlan = mealPlan.map((m) =>
        m.day === selectedMealInfo.day && m.mealType === selectedMealInfo.type
          ? { ...m, recipe: updatedRecipe }
          : m
      );
      ...
      setToast({
        message: 'Meal regenerated and saved to Supabase!',
        type: 'success',
      });
    } catch (error: any) {
  ...
  const getShoppingList = () => {
    ...
    mealPlan.forEach((item) => {
      if (item.recipe.title.toLowerCase().includes('leftover')) return;
  ...
  const handleComparePrices = async () => {
    setToast({
      message: 'Price comparison is temporarily disabled while switching from Gemini to OpenAI.',
      type: 'error',
    });
  };
          ...
          <div className="p-8">
              ...
              <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-amber-700 leading-relaxed">
                    Please set your Supabase credentials to enable login and data persistence.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-4 mb-8 p-1 bg-[#f0fdf4] rounded-2xl">
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all ${
                  authMode === 'login' ? 'bg-white text-[#15803d] shadow-sm' : 'text-[#15803d]/40'
                }`}
              >
                Login
              </button>
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all ${
                  authMode === 'register' ? 'bg-white text-[#15803d] shadow-sm' : 'text-[#15803d]/40'
                }`}
              >
                Register
              </button>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-[#15803d]/60 ml-1">
                  Email Address
                </label>
                      ...
                      {MEAL_TYPES.map((type) => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-3 rounded-2xl border transition-all cursor-pointer h-32 flex flex-col justify-between ${
                              meal
                                ? 'bg-white border-[#dcfce7] hover:shadow-md'
                                : 'bg-dashed border-2 border-[#dcfce7] border-dashed opacity-50'
                            }`}
                          >
                                ...
                                {meal && (
                                  <button
                                    ...
                                    className={`p-1 rounded-full transition-colors ${
                                      favorites.some(
                                        (f) =>
                                          (f.recipeId || '').trim().toLowerCase() ===
                                          meal.recipe.title.trim().toLowerCase()
                                      )
                                        ? 'text-red-500 bg-red-50'
                                        : 'text-[#15803d]/30 hover:bg-[#f0fdf4]'
                                    }`}
                                  >
                                    <Heart
                                      size={14}
            ...
            >
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-[#dcfce7]">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                        ...
                        <div className="flex items-center gap-3">
                          {comparisonBudget !== null && (
                            <div className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 bg-emerald-100 text-emerald-700 border border-emerald-200">
                              Target: £{comparisonBudget}
                            </div>
                          )}
                          <span className="text-[10px] text-[#15803d]/40 font-normal uppercase tracking-wider">
                            Price comparison
                          </span>
                        </div>
                      </div>
                ...
                <div className="mb-10 p-6 bg-[#f0fdf4] rounded-2xl border border-[#dcfce7]">
                  ...
                  <form onSubmit={handleAddManualItem} className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                    <input
                      ...
                      required
                      className="bg-white px-4 py-2 rounded-xl border border-[#dcfce7] focus:outline-none focus:ring-2 focus:ring-[#15803d]/20"
                    />
                    <input
                      name="amount"
                      placeholder="Qty (e.g. 1, 500g)"
                      className="bg-white px-4 py-2 rounded-xl border border-[#dcfce7] focus:outline-none focus:ring-2 focus:ring-[#15803d]/20"
                    />
                    <select
                      name="category"
                    ...
                    <button
                      type="submit"
                      className="bg-[#15803d] text-white px-6 py-2 rounded-xl font-medium hover:bg-[#166534] transition-colors flex items-center justify-center gap-2"
                    >
                      <Plus size={18} /> Add
                    </button>
                  </form>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {Object.entries(getGroupedShoppingList())
                    .sort(([a], [b]) => a.localeCompare(b))
                    .map(([category, items]) => {
                      return (
                        <div key={category} className="space-y-4">
                          <h4 className="font-serif font-bold text-lg border-b border-[#dcfce7] pb-2">
                            {category}
                          </h4>
                            ...
                            {items.map((item: any, idx: number) => {
                              ...
                              return (
                                <div
                                  ...
                                  className={`flex items-center justify-between p-2 hover:bg-[#f0fdf4] rounded-lg transition-colors group cursor-pointer ${
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <div
                                      className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-colors ${
                                        isChecked
                                          ...
                                          : 'border-[#15803d]/30 group-hover:border-[#15803d]'
                                      }`}
                                    >
                                      <CheckCircle2
                                        size={14}
                                    ...
                                    <span className={`${isChecked ? 'line-through text-[#15803d]/60' : ''}`}>
                                      {item.name}
                                    </span>
                                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {favorites.map((fav) => (
                    ...
                    >
                      ...
                      <div className="mb-4">
                        <h3 className="font-bold text-lg leading-tight group-hover:text-[#15803d] transition-colors pr-8">
                          {fav.recipe.title}
                        </h3>
                        <div className="flex flex-wrap gap-1 mt-2">
                          {fav.recipe.tags.slice(0, 2).map((tag: string) => (
                            <span
                              key={tag}
                              className="text-[10px] bg-white px-2 py-0.5 rounded-full border border-[#dcfce7] uppercase font-bold text-[#15803d]/60"
                            >
                              {tag}
                            </span>
                          ))}
                ...
                <div className="flex justify-between items-center mb-8">
                  <div>
                    ...
                    <p className="text-[#15803d]/70">
                      Personalize meal plans for everyone in your household.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                  ...
                  {profiles.map((profile) => (
                    <div
                      ...
                      className={`p-6 rounded-2xl border transition-all ${
                        profile.isActive
                          ? 'bg-white border-[#15803d] shadow-md'
                          : 'bg-[#f0fdf4] border-[#dcfce7] opacity-70'
                      }`}
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${
                              profile.isActive ? 'bg-[#15803d]' : 'bg-[#15803d]/40'
                            }`}
                          >
                          ...
                          <div>
                            ...
                            <span className="text-xs uppercase tracking-widest font-bold text-[#15803d]/60">
                      ...
                      <div className="space-y-3 mb-6">
                        {profile.allergies.length > 0 && (
                          <div>
                            <span className="text-[10px] uppercase font-bold text-red-500/60 block mb-1">
                              Allergies
                            </span>
                        ...
                        {profile.dislikedIngredients.length > 0 && (
                          <div>
                            <span className="text-[10px] uppercase font-bold text-[#15803d]/60 block mb-1">
                              Dislikes
                            </span>
                            <div className="flex flex-wrap gap-1">
                              {profile.dislikedIngredients.map((d) => (
                                <span
                                  key={d}
                                  className="text-[10px] bg-white text-[#15803d] px-2 py-0.5 rounded-full border border-[#dcfce7]"
                                >
                                  {d}
                                </span>
                              ))}
          ...
          <>
            ...
            >
                ...
                {Object.entries(getGroupedShoppingList())
                  ...
                  .map(([category, items]) => (
                    <div key={category} className="space-y-3">
                      <h4 className="font-bold text-sm uppercase tracking-widest text-[#15803d]/40 border-b border-[#dcfce7] pb-1">
                        {category}
                      </h4>
                      <div className="space-y-2">
                        {items.map((item: any, idx: number) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-3 bg-[#f0fdf4]/50 rounded-xl border border-[#dcfce7] hover:border-[#15803d]/30 transition-all"
                          >
                            <div>
                              <p className="font-bold text-sm">{item.name}</p>
                              <p className="text-xs text-[#15803d]/60">
              ...
              <div className="p-6 border-t border-[#dcfce7] bg-[#f0fdf4]">
                <button
                  onClick={() => {
                    const listText = getShoppingList()
            ...
            >
              <div className="p-8">
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-bold text-[#15803d]/60 uppercase tracking-widest mb-2">
                      Weekly Budget (£)
                    </label>
                    <input
                      type="number"
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 bg-[#f0fdf4] p-2 rounded-xl border border-[#dcfce7]">
                        <button
                          ...
                          className="w-10 h-10 rounded-lg bg-white flex items-center justify-center border border-[#dcfce7] hover:bg-[#dcfce7]"
                        >
                          -
                        </button>
                ...
                onSubmit={(e) => {
                  ...
                  const profile: UserProfile = {
                    ...
                    allergies: (formData.get('allergies') as string)
                      ...
                      .map((s) => s.trim())
                      .filter(Boolean),
                    dislikedIngredients: (formData.get('dislikes') as string)
                      .split(',')
                      .map((s) => s.trim())
                      .filter(Boolean),
              ...
              <div className="p-8 overflow-y-auto">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 text-sm px-3 py-1 rounded-full transition-colors ${
                          favorites.some(
                            (f) =>
                              (f.recipeId || '').trim().toLowerCase() ===
                              selectedRecipe.title.trim().toLowerCase()
                          )
                            ? 'text-red-500 bg-red-50'
                            : 'text-[#15803d]/70 bg-[#f0fdf4] hover:bg-[#dcfce7]'
                        }`}
                      >
                        <Heart
                          size={14}
                ...
                {selectedRecipe.nutrition && (
                  <div className="space-y-2 mb-8">
                    <p className="text-xs font-bold text-[#15803d]/60 uppercase tracking-widest ml-1">
                ...
                <div className="mt-12 space-y-8">
                  <div className="border-t border-[#dcfce7] pt-8">
                    ...
                    <form
                      ...
                      className="bg-[#f0fdf4] p-6 rounded-2xl border border-[#dcfce7] mb-8"
                    >
                      ...
                      <button
                        type="submit"
                        className="bg-[#15803d] text-white px-6 py-2 rounded-full font-medium hover:bg-[#166534] transition-colors"
                      >
                        Submit Review
                      </button>
                    </form>
                    ...
                    <div className="space-y-4">
                      {reviews.map((review) => (
                        <div key={review.id} className="bg-white p-4 rounded-2xl border border-[#dcfce7]">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className="text-[10px] text-[#15803d]/40">
                      ...
                      {reviews.length === 0 && (
                        <p className="text-center text-[#15803d]/40 py-4">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#dcfce7] z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
        <div className="max-w-5xl mx-auto px-6 py-2 flex justify-start gap-2 sm:gap-6">
          <button
            ...
            className={`flex flex-col sm:flex-row items-center gap-1 sm:gap-3 px-4 py-2 rounded-2xl transition-all ${
              activeTab === 'planner'
                ? 'text-[#15803d] bg-[#f0fdf4]'
                : 'text-[#15803d]/40 hover:text-[#15803d]/60 hover:bg-[#f0fdf4]/50'
            }`}
          >
